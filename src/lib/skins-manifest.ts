...
export interface GallerySkin extends SkinManifestEntry {
  width: number;
  height: number;
  config: SkinConfig;
...
export function parseManifestEntry(raw: unknown): SkinManifestEntry {
  ...
  return {
    id,
    title: requireString(entry.title, "title", id),
    author: requireString(entry.author, "author", id),
    authorUrl: typeof entry.authorUrl === "string" ? entry.authorUrl : undefined,
    origin: origin as SkinOrigin,
    description: requireString(entry.description, "description", id),
